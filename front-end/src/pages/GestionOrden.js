...
function GestionOrden() {
    ...
    if (id && !location.state?.orden) {
      ...
      setFormData({
        ...
        Fecha_Hora: orden.Fecha_Hora
          ? new Date(orden.Fecha_Hora).toISOString().slice(0, 16)
          : "",
  ...
  const cargarDatosOrden = async (ordenId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/orden/read/${ordenId}`
      );
      if (!response.ok) {
      ...
      setFormData({
        ...
        Fecha_Hora: ordenData.Fecha_Hora
          ? new Date(ordenData.Fecha_Hora).toISOString().slice(0, 16)
          : "",
  ...
  const handleCreateOrUpdateSubmit = async (event) => {
    ...
    const url = editing
      ? `http://localhost:5000/orden/update/${formData.ID_Orden}`
      : "http://localhost:5000/orden/create";
    ...
    <div className="body-content">
      <Container className="mt-custom">
        <Card>
          <Card.Body>
            <Card.Title>
              {editing ? "Editar Orden" : "Crear Nueva Orden"}
            </Card.Title>
            {responseMessage && (
              <Alert variant={responseMessage.type}>
